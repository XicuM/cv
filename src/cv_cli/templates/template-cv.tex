\documentclass[$if(style.fontsize)$$style.fontsize$$else$10pt$endif$,a4paper]{article}

% ----------------------------------------------------------------------
% Required packages

\usepackage{graphicx}       % To include images
\usepackage{xcolor}         % To add colour to the document
\usepackage{marvosym}       % Provides icons for the contact details
\usepackage{fontawesome5}   % FontAwesome icons (includes material design style)
\usepackage{enumitem}       % To redefine spacing in lists
\usepackage[T1]{fontenc}    % Output font encoding
\usepackage[utf8]{inputenc} % Input encoding
\usepackage[scaled]{helvet} % Helvetica font
\usepackage{tikz}           % For drawing badges
\usepackage{tikzpagenodes}  % For page nodes
\usepackage{eso-pic}        % For background colors and images
\usepackage[
    left=0mm, 
    right=0mm, 
    top=0mm, 
    bottom=0mm
]{geometry}                 % Page dimensions
\usepackage[
    stretch=25, 
    shrink=25, 
    tracking=true, 
    letterspace=30
]{microtype}                % Better typography
\usepackage[
    colorlinks=true, 
    urlcolor=black, 
    linkcolor=black
]{hyperref}                 % For hyperlinks

% Load TikZ libraries
\usetikzlibrary{tikzmark}
\usetikzlibrary{backgrounds}

% Declare layers
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background, main, foreground}

% ----------------------------------------------------------------------
% Custom commands

% Custom blue bullet
\newcommand{\bluebullet}{%
    \hspace{0.09cm}%
    \tikz[baseline=-0.14cm]{%
        \fill[white] (0,0) circle (0.16cm);%
        \fill[primary] (0,0) circle (0.08cm);%
    }%
}

% Custom hollow bullet
\newcommand{\hollowbullet}{%
    \makebox[0pt][l]{%
        \hspace{-0.4cm}%
        \raisebox{0.04cm}{%
            \tikz[baseline=(char.base)]{%
                \draw[secondary] (0,0) circle (0.06cm);%
            }%
        }%
    }%
}

% Date command
\newcommand{\dates}[2]{%
    \hfill\raisebox{0.04cm}{\footnotesize\color{primary}\textbf{#1 - #2}}%
}

% Reusable boxes to measure wrapped title height
\newsavebox{\entrytitlebox}
\newsavebox{\subentrytitlebox}

% Entry header with wrapped title and fixed date column
\newcommand{\entryheader}[3]{%
    \sbox{\entrytitlebox}{\parbox[t]{0.60\linewidth}{\raggedright\textsc{#1}}}%
    \typeout{DEBUG TITLE: #1, HEIGHT: \the\ht\entrytitlebox, DEPTH: \the\dp\entrytitlebox, THRESHOLD: \the\dimexpr1.3\baselineskip\relax}%
    \noindent\makebox[0pt][l]{\bluebullet\hspace{0.20cm}}%
    \hspace{0.65cm}%
    \begin{minipage}[t]{0.60\linewidth}%
        \raggedright\textsc{#1}%
    \end{minipage}%
    \hfill%
    \hspace{0.06cm}%
    \begin{minipage}[t]{0.27\linewidth}%
        \raggedleft\raisebox{0.04cm}{\footnotesize\color{primary}\textbf{#2 - #3}}%
    \end{minipage}\par%
    \ifdim\dimexpr\ht\entrytitlebox+\dp\entrytitlebox\relax>1.3\baselineskip
        \vspace{\dimexpr\ht\entrytitlebox+\dp\entrytitlebox-1.1\baselineskip+0.08cm\relax}%
    \else
        \vspace{0.08cm}%
    \fi%
}

% Sub-entry header with wrapped title and fixed date column
\newcommand{\subentryheader}[3]{%
    \sbox{\subentrytitlebox}{\parbox[t]{0.56\linewidth}{\raggedright\textsc{#1}}}%
    \noindent\hspace{1cm}%
    \begin{minipage}[t]{0.56\linewidth}%
        \raggedright\textsc{#1}%
    \end{minipage}%
    \hfill%
    \hspace{0.10cm}%
    \begin{minipage}[t]{0.27\linewidth}%
        \raggedleft\raisebox{0.04cm}{\footnotesize\color{primary}\textbf{#2 - #3}}%
    \end{minipage}\par%
    \ifdim\dimexpr\ht\subentrytitlebox+\dp\subentrytitlebox\relax>1.3\baselineskip
        \vspace{\dimexpr\ht\subentrytitlebox+\dp\subentrytitlebox-1.1\baselineskip+0.08cm\relax}%
    \else
        \vspace{0.08cm}%
    \fi%
}

% Section header for left column
\newcommand{\headleft}[1]{%
    \vspace*{0.4cm}\textsc{\textbf{#1}}\par%
    \vspace*{-1.1ex}\hrulefill\par\vspace*{0.12cm}%
}

% Tighter first section header (used below the name block)
\newcommand{\headleftfirst}[1]{%
    \vspace*{0.16cm}\textsc{\textbf{#1}}\par%
    \vspace*{-1.1ex}\hrulefill\par\vspace*{0.12cm}%
}

% Section header for right column
\newcommand{\headright}[1]{%
    \textsc{\large\color{primary}#1}\par%
    \vspace*{-0.25cm}{\color{primary}\hrulefill}\par%
    \vspace*{0.25cm}%
}

% Counter for unique section identifiers
\newcounter{sectioncounter}

% Counter for unique subposition identifiers
\newcounter{subposcounter}

% Section with vertical line
\newcommand{\newsection}[2]{%
    \stepcounter{sectioncounter}%
    \headright{#1}%
    \tikzmark{start-\arabic{sectioncounter}}%
    #2%
    \par\noindent%
    \tikzmark{end-\arabic{sectioncounter}}%
}

% Detail item command
\newcommand{\detail}[1]{%
    {\small\color{secondary}#1}%
}

% Company/location line for entries and sub-entries
\newcommand{\companyline}[3]{%
    \hspace{#1}#2#3\par\vspace{0.1cm}%
}

% Reusable details list with configurable left margin
\newcommand{\detailslist}[2][1.12cm]{%
    \begin{itemize}[leftmargin=#1]%
        #2%
    \end{itemize}%
}

% Draw vertical line for a sub-position timeline
\newcommand{\drawsubpositionline}[2]{%
    \vbox to 0pt{%
        \begin{tikzpicture}[remember picture, overlay]%
            \coordinate (line-start-subpos-\arabic{subposcounter}) at ([xshift=0.70cm, yshift=#1]pic cs:start-subpos-\arabic{subposcounter});%
            \coordinate (line-end-subpos-\arabic{subposcounter}) at ([xshift=0.70cm, yshift=#2]pic cs:end-subpos-\arabic{subposcounter});%
            \draw[primary!30, line width=1.8pt] (line-start-subpos-\arabic{subposcounter}) -- (line-end-subpos-\arabic{subposcounter});%
        \end{tikzpicture}%
        \vss%
    }%
}

% Reusable subposition block
\newcommand{\subpositionentry}[7]{%
    \stepcounter{subposcounter}%
    \tikzmark{start-subpos-\arabic{subposcounter}}%
    \subentryheader{#1}{#2}{#3}%
    #4%
    \detailslist[1.5cm]{#5}%
    \tikzmark{end-subpos-\arabic{subposcounter}}%
    \par\vspace{-0.55cm}%
    \drawsubpositionline{#6}{#7}%
}

% Badge command
\newcommand{\skillbadge}[1]{%
    \tikz[baseline=(char.base)]{%
        \node[%
            shape=rectangle, %
            rounded corners=3pt, %
            inner sep=2pt, %
            fill=primary!10%
        ] (char) {\small\textbf{#1}};%
    }%
}

% Skill category label
\newcommand{\skillcategory}[1]{%
    {\small\textit{#1}}\\%
}

% Circular image command with diffused shadow
\newcommand{\circularimage}[2][2.5cm]{%
    \begin{tikzpicture}%
        % Diffused shadow
        \fill[black!15] (0.08,-0.08) circle (#1/2 + 0.3mm);%
        \fill[black!20] (0.06,-0.06) circle (#1/2 + 0.2mm);%
        \fill[black!25] (0.04,-0.04) circle (#1/2 + 0.1mm);%
        \fill[black!30] (0.02,-0.02) circle (#1/2);%
        % Main circular image
        \begin{scope}%
            \clip (0,0) circle (#1/2);%
            \node[anchor=center] at (0,0) {\includegraphics[width=#1]{#2}};%
        \end{scope}%
    \end{tikzpicture}%
}

% ----------------------------------------------------------------------
% Set style

% Colors
\definecolor{primary}{HTML}{$if(style.primary_color)$$style.primary_color$$else$000000$endif$}
\definecolor{secondary}{HTML}{$if(style.secondary_color)$$style.secondary_color$$else$666666$endif$}

% Make links use the same tone as detail text
\hypersetup{urlcolor=secondary, linkcolor=secondary}

% Set sans-serif as the default font family
\renewcommand{\familydefault}{\sfdefault}

% List settings
\setlist{
    parsep=0pt, 
    topsep=0pt, 
    partopsep=0pt, 
    itemsep=0.1cm, 
    leftmargin=1.12cm,
    labelsep=0cm,
    label=\hollowbullet
}

% Margins
\newlength{\rightcolumnleftmargin}\setlength{\rightcolumnleftmargin}{0.5cm}
\newlength{\rightcolumnrightmargin}\setlength{\rightcolumnrightmargin}{0.5cm}
\newlength{\leftcolumnleftmargin}\setlength{\leftcolumnleftmargin}{0.6cm}
\newlength{\leftcolumnrightmargin}\setlength{\leftcolumnrightmargin}{0.5cm}

% Page background (including colored left column and vertical timeline)
\AddToShipoutPictureBG{%
    \begin{tikzpicture}[remember picture,overlay]%
        \fill[primary!20]%
            (current page.north west) rectangle ([xshift=0.33\paperwidth]current page.south west);%
        \begin{pgfonlayer}{background}%
            \foreach \n in {1,...,\value{sectioncounter}}{%
                \draw[primary!30, line width=2.6pt] 
                    ([xshift=0.25cm,yshift=0cm]pic cs:start-\n) -- 
                    ([xshift=0.25cm,yshift=0.75cm]pic cs:end-\n);%
            }%
        \end{pgfonlayer}%
    \end{tikzpicture}%
}

% No indentation, no paragraph spacing, no box padding
\setlength{\parindent}{0pt}
\setlength{\parskip}{0pt}
\setlength{\fboxsep}{0pt}

% ----------------------------------------------------------------------
% Main document
 
\begin{document}%
    \vspace*{-0.65cm}%
    \noindent\begin{tabular}[t]{%
        @{}p{0.33\textwidth}@{\hskip0.01\textwidth}p{0.63\textwidth}@{}%
    }%
        % --------------------------------------------------------------
        % Left column

        \hspace{\leftcolumnleftmargin}%
        \begin{minipage}[t]{%
            \dimexpr0.325\textwidth-2\leftcolumnleftmargin%
        }%
            \raggedright%
            \vspace*{0.75cm}%

            \begin{center}

                $if(assets.picture)$ % Profile picture %
                    \circularimage[3.5cm]{../../../assets/$assets.picture$} \\[0.3cm]
                $endif$
                $if(assets.signature)$ % Signature %
                    \includegraphics[
                        width=$assets.signature_width$
                    ]{../../../assets/$assets.signature$} \\[0.2cm]
                $endif$

                $if(name)$ % Name %
                    \large \textbf{$name$}
                $endif$
            \end{center}

            $if(profile)$ % Profile summary %
                \headleftfirst{$lang_profile$}
                $for(profile)$ $profile$$sep$\\[0.8ex]$endfor$
            $endif$

            % Contact details
            \headleft{$lang_contact$}
            {%
                \small%
                $if(contact.location)$ % Location %
                    \faMapMarker\ \ $contact.location$ \\[0.5ex]
                $endif$%
                $if(contact.email)$ % Email %
                    \faEnvelope\ \ {\small \href{mailto:$contact.email$}{$contact.email$}} \\[0.5ex]
                $endif$%
                $if(contact.phone)$ % Phone %
                    \faPhone\ \ $contact.phone$ \\[0.5ex]
                $endif$%
                $if(contact.linkedin)$ % LinkedIn %
                    \faLinkedin\ \ \href{https://$contact.linkedin$}{$contact.linkedin$} \\[0.5ex]
                $endif$%
                $if(contact.github)$ % GitHub %
                    \faGithub\ \ \href{https://$contact.github$}{$contact.github$} \\[0.5ex]
                $endif$%
                $if(contact.website)$ % Website %
                    \faGlobe\ \ \href{https://$contact.website$}{$contact.website$} \\[0.5ex]
                $endif$%
            }

            % Personal information
            \headleft{$lang_personal_info$}
            {%
                $if(personal_info.birthdate)$ % Birthdate %
                    $lang_birthdate$: \textbf{$personal_info.birthdate$} \\[0.5ex]
                $endif$
                $if(personal_info.place_of_birth)$ % Place of birth %
                    $lang_place_of_birth$: \textbf{$personal_info.place_of_birth$} \\[0.5ex]
                $endif$
                $if(personal_info.citizenship)$ % Citizenship %
                    $lang_citizenship$: \textbf{$personal_info.citizenship$} \\[0.5ex]
                $endif$%
                $lang_languages$: \\[0.4ex] % Languages %
                \begin{itemize}[leftmargin=0.4cm, itemsep=0pt, label=-\ ]
                    $for(personal_info.languages)$
                        \item \textbf{$it.name$}~($it.level$)
                    $endfor$
                \end{itemize} \par\vspace{0.7ex}
                $if(personal_info.extra)$ % Additional personal information %
                    $for(personal_info.extra)$
                        $it$\\[0.5ex]
                    $endfor$%
                $endif$%
            }
            
            % Key skills
            $if(key_skills)$\headleft{$lang_key_skills$}{
                \vspace*{0.2cm}%
                \parbox{\linewidth}{%
                    \raggedright \linespread{1.08}\selectfont%
                    $for(key_skills)$%
                        $if(it.category)$%
                            % categorized, skip in first loop
                        $else$%
                            $if(it.items)$%
                                $for(it.items)$\skillbadge{$it$}\hspace{0.09cm}$endfor$\\[0.1cm]%
                            $else$%
                                \skillbadge{$it$}\hspace{0.09cm}%
                            $endif$%
                        $endif$%
                    $endfor$%
                    $for(key_skills)$%
                        $if(it.category)$%
                            \skillcategory{$it.category$}%
                            $for(it.items)$\skillbadge{$it$}\hspace{0.09cm}$endfor$\\[0.1cm]%
                        $endif$%
                    $endfor$%
                }%
            }
            $endif$

        \end{minipage} &

        % --------------------------------------------------------------
        % Right column

        \setlength{\parskip}{0.25ex}%
        \hspace{\rightcolumnleftmargin}%
        \begin{minipage}[t]{%
            \dimexpr0.63\textwidth-\rightcolumnleftmargin-\rightcolumnrightmargin%
        }%
            \raggedright%
            \vspace*{0.9cm}%

            % ----------------------------------------------------------
            % Professional experience
            
            $if(professional_experience)$\newsection{$lang_professional_experience$} {%
                $for(professional_experience)$%
                    % Position, company, location and dates
                    \entryheader{$it.position$}{$it.dates.start$}{$it.dates.end$}%
                    \companyline{0.65cm}{$it.company$}{$if(it.location)$ - \textit{$it.location$}$endif$}%
                    % Position details
                    $if(it.details)$
                        \detailslist{%
                            $for(it.details)$ \item \detail{$it$} $endfor$%
                        }%
                    $endif$%
                    % Subpositions (for internal promotions or multiple roles in the same company)
                    $if(it.subpositions)$%
                        \vspace{0.13cm}
                        $for(it.subpositions)$%
                            \subpositionentry{$it.position$}{$it.dates.start$}{$it.dates.end$}{%
                                $if(it.company)$%
                                    \companyline{1cm}{$it.company$}{$if(it.location)$ - \textit{$it.location$}$endif$}%
                                $else$%
                                    \vspace{0.1cm}%
                                $endif$%
                            }{%
                                $for(it.details)$ \item \detail{$it$} $endfor$
                            }{0.3cm}{0.50cm}%
                        $endfor$%
                        \vspace{-0.10cm}
                    $endif$ $if(it.subpositions)$\vspace{0.08cm}$else$\vspace{0.3cm}$endif$
                $endfor$%
            }%
            $endif$
            
            % ----------------------------------------------------------
            % Additional experience (projects, internships, volunteering, etc.)
            
            $if(additional_experience)$\newsection{$lang_additional_experience$} {%
                $for(additional_experience)$%
                    % Position, company, location and dates
                    \entryheader{$it.position$}{$it.dates.start$}{$it.dates.end$}%
                    \companyline{0.65cm}{$it.company$}{$if(it.location)$ - \textit{$it.location$}$endif$}%
                    % Position details
                    $if(it.details)$
                        \detailslist{%
                            $for(it.details)$ \item \detail{$it$} $endfor$%
                        }%
                    $endif$%
                    % Subpositions (for internal promotions or multiple roles in the same company)
                    $if(it.subpositions)$%
                        \vspace{0.13cm}
                        $for(it.subpositions)$%
                            \subpositionentry{$it.position$}{$it.dates.start$}{$it.dates.end$}{%
                                $if(it.company)$%
                                    \companyline{1cm}{$it.company$}{$if(it.location)$ - \textit{$it.location$}$endif$}%
                                $else$%
                                    \vspace{0.1cm}%
                                $endif$%
                            }{%
                                $for(it.details)$ \item \detail{$it$} $endfor$
                            }{0.32cm}{0.42cm}%
                        $endfor$%
                        \vspace{-0.10cm}
                    $endif$ $if(it.subpositions)$\vspace{0.08cm}$else$\vspace{0.3cm}$endif$
                $endfor$%
            }
            $endif$
            
            % ----------------------------------------------------------
            % Education

            $if(education)$\newsection{$lang_education$} {%
                $for(education)$%
                    \entryheader{$it.degree$}{$it.dates.start$}{$it.dates.end$}%
                    \companyline{0.65cm}{$it.institution$}{$if(it.location)$ - \textit{$it.location$}$endif$}%
                    $if(it.details)$
                    \detailslist{%
                        $for(it.details)$ \item \detail{$it$} $endfor$%
                    }%
                    $endif$
                    \vspace{0.3cm}%
                $endfor$%
            }
            $endif$

            \end{minipage}%
    \end{tabular}%
\end{document}
